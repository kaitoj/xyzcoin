...
#if defined(NDEBUG)
# error "Xyzcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2b379f242af728d7ad18df7d8b7f6d7ee0a987780fcdab56356c001ec0be6082");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Xyzcoin: starting difficulty is 1 / 2^12
...
{
    // Xyzcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Xyzcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Xyzcoin: 3.5 days
static const int64 nTargetSpacing = 1.5 * 60; // Xyzcoin: 90 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Xyzcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Xyzcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3acbbcc500475f31853c8912fda7fdcef59976bec5ba4ca5ab91da04ceab0048");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "21/03/2021 - Year 2 of COVID-19";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04ac73a785a3295679d3bd461a927c82cd32dbb54b7c1edb56d68386f66cd74d65799a2e09fb2c954eb5fae9719dc678191fd6dbfe2d6c788187e18660fd5cf5a4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1616329319;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087006351;

        if (fTestNet)
        {
            block.nTime    = 1616329349;
            block.nNonce   = 387829394;
        }

        // Set true to get genesis block
        ...
        assert(block.hashMerkleRoot == uint256("0x2937ed7abec9e83be44f7bd55ac89f3b62707234dff0a4401b72c69e99a809d2"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xca, 0xba, 0xda }; // Xyzcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("XyzcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("XyzcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static XyzcoinMiner(CWallet *pwallet)
{
    printf("XyzcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running XyzcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("XyzcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&XyzcoinMiner, pwallet));
}
